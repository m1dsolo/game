...
public:
    ...
    SDL_Surface* create_surface(const std::string& text, SDL_Color color = BLACK);
    SDL_Texture* create_texture(SDL_Surface* surface);
    SDL_Texture* create_texture(const std::string& text, SDL_Color color = BLACK);
    ...
    void draw_text(const std::string& text, const SDL_FRect* dst, SDL_Color color, bool mid = false);
